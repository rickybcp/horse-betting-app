...
def migrate_local_to_cloud():
    ...
    """
    print("🚀 Starting migration from local storage to Google Cloud Storage")
    print("=" * 60)
...
def verify_cloud_data():
    ...
    """
    print("\n🔍 Verifying cloud storage data...")
    
    storage_manager = get_storage_manager()
    
    if not storage_manager.use_cloud:
        print("❌ Cloud storage not available for verification")
        return False
        ...
        try:
            ...
            if data is not None:
                ...
                if filepath == "data/users.json" and isinstance(data, list):
                    print(f"     📊 Contains {len(data)} users")
                ...
                # Show some stats for race day files
                    ...
                    print(f"     📊 {races_count} races, {bets_count} users with bets, {bankers_count} bankers")
            else:
    ...
    print(f"\n📋 Verification complete: {verified_files}/{total_key_files} key files found")
    
    if verified_files == total_key_files:
        print("🎉 All key files verified successfully!")
        return True
    else:
        print("⚠️ Some key files are missing from cloud storage")
        return False
...
def list_cloud_files():
    ...
    """
    print("\n📂 Listing all files in cloud storage...")
...
if __name__ == "__main__":
    print("🏇 Lekours Data Migration Tool")
    print("=" * 60)
